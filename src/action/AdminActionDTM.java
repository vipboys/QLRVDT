...
import bo.AdminBODTM;
import form.AdminFormDTM;
import utils.MyException;

public class AdminActionDTM extends Action {
/*	private String captchaCode;*/
	/*public static String address="C\:\\Users\\HCD-Fresher203\\Documents\\Java\\QLRVDT\\WebContent";*/
	public static String address="D:\\images";
	@Override
	public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		AdminFormDTM formDTM=(AdminFormDTM)form;
		...
		if(formDTM.getId_account()!=0){
			...
			try {				
				if(formDTM.getType_post().trim().equals("VIP"))
					id_post=AdminBODTM.createPost(formDTM.getId_account(),formDTM.getTitle_post(),formDTM.getContent_post(),formDTM.getType_product(),formDTM.getPercent_product(),formDTM.getCost(),simpleDateFormat.format(date),formDTM.getAddress_product(),simpleDateFormat.format(date),formDTM.getType_post(),0,"FALSE",90);
				else
					id_post=AdminBODTM.createPost(formDTM.getId_account(),formDTM.getTitle_post(),formDTM.getContent_post(),formDTM.getType_product(),formDTM.getPercent_product(),formDTM.getCost(),simpleDateFormat.format(date),formDTM.getAddress_product(),simpleDateFormat.format(date),formDTM.getType_post(),0,"FALSE",30);
			} catch (MyException e) {
			    ...
			    if(formDTM.getUpload()!=null){
			    	try {
				    	for(int i=0;i<formDTM.getUpload().size();i++){
						    ...
						    try {
								AdminBODTM.createImages(id_post,"images\\"+id_post+"\\"+fileName);
							} catch (MyException e) {								
								e.printStackTrace();
